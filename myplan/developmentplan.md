# AEI Student Upload - Development Plan
...
- `uv init` the backend project; add FastAPI, SQLModel, Uvicorn, openpyxl,
  python-multipart, pytest as dependencies.
- Scaffold the Next.js + TypeScript + Tailwind frontend, configured for static
  export (`output: 'export'`) into `frontend/out`.
- Establish the folder layout above; add `.env.example` for the SQLite DB path, host
  and port (default port `8008`) - no port or URL hardcoded in source.
- Wire FastAPI's `main.py` to mount the API routers under `/api` and serve
  `frontend/out` via `StaticFiles` for every other route.
- Confirm `uv run uvicorn app.main:app --port 8008` serves a placeholder page and a
  placeholder `/api/health` route from the one process/port.
