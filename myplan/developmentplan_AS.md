# Authorized Signatory - Development Plan
...
### Phase 1 - Data Model
- Add `MasterApplicant` and `AuditRecord` SQLModel tables to `models.py`.
- Add seed loader for `masterapplicants.csv` into `db.py`, with the header
  typo-mapping noted above; verify 24 rows loaded (16 active / 8 inactive) and spot
  checks against the CSV.
- `audit_records` stays empty at startup - verify no seed rows are inserted.
